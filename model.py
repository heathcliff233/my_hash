...
class MyEncoder(nn.Module):
    def __init__(self, k=3, in_channels=21, out_channels=21 ** 3):
        ...
        self.rot_mx = torch.from_numpy(rot_mx).cuda()

    def forward(self, x):
        x = x.permute(0, 2, 1).unsqueeze(3)
        x = torch.matmul(self.rot_mx, x).squeeze(3).permute(0, 2, 1)
        x1 = self.hash1(x)
        x2 = self.hash2(x)
        x3 = self.hash3(x)
        x = x1 * x2 * x3
    ...
    def get_loss(self, ebd):
        ...
        loss1 = F.nll_loss(sm_score, label.to(sm_score.device), reduction="mean")
    ...
    def gen_ebd(self, data, lens):
        with torch.no_grad():
            ...
            res = torch.stack(
                [data[i][: lens[i] - 1].sum(dim=0) for i in range(len(lens))]
            )
        return res
